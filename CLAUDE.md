...
- **Click = flip** (280ms timer), **double-click = rotate** (350ms window). Both bank and tray pieces support click/double-click. Uses `gameState.clickPieceId` in the timer callback to avoid closure scope bugs.
- **Tray double-click pivot rotation**: double-clicking a tray piece rotates it 90° CW around the clicked cell (`rotatePieceAt()`). The piece's grid position is adjusted so the clicked cell remains in place after rotation. Standard `rotatePiece()` is used for bank pieces.
- **Overlap validation**: flip and rotate operations on tray pieces check whether the resulting position would overlap blocked cells or other pieces (`wouldOverlap()`). If a conflict is detected the operation is silently rejected.
- **No click-to-pickup**: clicking a placed piece flips it — dragging is the only way to remove a piece from the tray.
...
- **Guard against spurious drag**: bank and canvas `pointermove` handlers check `gameState.clickTimer` to ignore movements that arrive after pointer-up (between click and the 280ms timer). Without this guard, post-click mouse movement could trigger a drag transition with no active pointer capture to clean up, permanently breaking interaction.
...
The help dialog (`#help-overlay`) explains controls (drag, click-to-flip, double-click-to-rotate) and the win condition. Opened via the `?` button in the header. Content uses `data-i18n` keys: `help_title`, `help_controls_title`, `help_ctrl_drag/rotate/flip/return`, `help_goal_title`, `help_goal`. A footer line says "Generated by DeepSeek V4 Pro, designed by hanasaka.net" with a clickable link (opens in new tab).

### Visitor counter footer

The page footer (`#site-footer`) shows site-wide page views and unique visitors powered by **busuanzi** (不蒜子). The counter script loads asynchronously from `busuanzi.ibruce.info` (loaded in `<head>`). Labels use `data-i18n` keys `stats_views` and `stats_visitors` and update with language changes. Before the script loads, counters show `-` as fallback. Styled subtly at 60% opacity using `--text-secondary`; count numbers use `--accent`. Each counter has an inline SVG icon (eye for views, users for visitors) with `stroke="currentColor"` matching the page's icon style.
...
Month/weekday display data lives in `I18N_MONTHS` and `I18N_WEEKDAYS` objects, keyed by language code. Each language also has `cal_weekdays` in TRANSLATIONS for the calendar grid headers. Visitor counter labels use `stats_views` and `stats_visitors` keys.
